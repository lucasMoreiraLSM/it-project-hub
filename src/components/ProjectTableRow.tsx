...
export const ProjectTableRow: React.FC<ProjectTableRowProps> = ({
  ...
  return (
    <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => onSelectProject(project)}>
      ...
      <TableCell className="text-center">
        <Badge variant={project.inovacaoMelhoria === 'Inovação' ? 'default' : 'secondary'} className="text-xs">
          {project.inovacaoMelhoria === 'Inovação' ? 'Inov' : 'Melh'}
        </Badge>
      </TableCell>
      <TableCell className="text-center text-sm">{statusGeral}</TableCell>
      <TableCell className="text-center">
        <Badge variant={project.estrategicoTatico === 'Estratégico' ? 'default' : 'outline'} className="text-xs">
      ...
      <TableCell className="text-center">
        <div className="flex gap-1 justify-center">
          <Button 
            variant="outline" 
            size="sm" 
            onClick={e => onShowReport(project, e)} 
